...
#sed -i 's|/var/www/html/pandora_console|/data/pandora/pandora_console|g' "$0"
...
mysql -u$DBUSER -P$DBPORT -h$DBHOST $DBNAME < /var/www/html/pandora_console/pandoradb.sql &>> "$LOGFILE"
check_cmd_status 'Error Loading database schema'

echo -en "${cyan}Loading pandoradb_data.sql to $DBNAME database...${reset}"
mysql -u$DBUSER -P$DBPORT -h$DBHOST $DBNAME < /var/www/html/pandora_console/pandoradb_data.sql &>> "$LOGFILE"
check_cmd_status 'Error Loading database schema data'

# Configure console
# Set console config file
cat > /var/www/html/include/pandora_console/config.php << EO_CONFIG_F
<?php
\$config["dbtype"] = "mysql";
